...
- Black-box abstraction :: Encapsulating an operation so the details of it are irrelevant.
  ...
  2. Combination: combine procedures with /composition/, combine data with /construction/ of compound data.
...
parameter of a procedure whose name is irrelevant is called a *bound variable*.
A procedure definition *binds* its parameters. A *free variable* isn't bound.
The set of expressions in which a binding defines a name is the *scope* of that
...
- Tree-recursive :: means in general, time is proportional to the number of nodes, space
...
I'm not going to include the full printout of the src_scheme{(count-change 11)}, here's an
example of what this looks like via ~1~.
...
#+RESULTS[cc8e697f3f3cb1bd72b524d3ef0570805bf1d0b7]:
:results:
#+begin_example
