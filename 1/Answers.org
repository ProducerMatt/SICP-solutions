...
#+NAME: 1-22-smd
| 5396.105071 | 20821.150618 | 53665.47429 | 122047.742337 |
...
#+RESULTS[76ee27a660faaba8b6218dc3f144a22f56305544]: 1-22-smd-massage
:results:
|    1000 |   5396.105071 |
|   10000 |  20821.150618 |
|  100000 |   53665.47429 |
| 1000000 | 122047.742337 |
:end:
...
#+RESULTS[b58014bd4e9d73dc4e68ec53ce3fc2fbcf165167]:
:results:
[[file:fig/1-22-1.png]]
:end:
...
#+NAME: 1-22-smdf
| 6366.99462 | 24845.9193 | 64756.73693 | 143869.63561 |

#+NAME: 1-22-smdf-massage
#+BEGIN_SRC scheme :eval no-export :results output table :var smd=1-22-smd smdf=1-22-smdf :export results :rownames no :colnames no
;(map (lambda(x y) (list x y))
<<print-table>>
(use-srfis '(1))
(print-table (zip
     '("min" 1000 10000 100000 1000000)
     (cons "(+ 1)" (car smd))
     (cons "(next)" (car smdf)))
             #:colnames #t)
#+END_SRC

#+RESULTS[6abc5093fcb47049c1f59787f38f46df0ab29317]: 1-22-smdf-massage
:results:
|     min |        (+ 1) |       (next) |
|---------+--------------+--------------|
...
#+begin_src gnuplot :var data=1-22-smdf-massage :file fig/1-22-2.png :exports both :eval no-export :rownames no :colnames no
reset
set logscale x
set xlabel 'minimum number'
set ylabel 'time'
...
plot data using 1:2 with linespoints title '(+ 1)', \
     data using 1:3 with linespoints title '(next)', \
     data using 1:(f($1)) with linespoints title \
     sprintf('sqrt(x * %.2f + 1000)', a)
#+end_src

#+RESULTS[c322c2d0a246417bd67c453d08745cfeee31ef67]:
:results:
[[file:fig/1-22-2.png]]
:end:
