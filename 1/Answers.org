...
colnames.
#+NAME: print-table
#+BEGIN_SRC scheme -n :eval no-export :tangle yes :results output :exports code
...
#+BEGIN_SRC scheme -n :eval no-export :tangle yes :results table output :exports no :rownames yes :colnames yes
<<print-table>>
(let* ((l (iota 3))
      (table (list
...
#+begin_src gnuplot :var data=1-24-massage :file fig/1-24-1.png :exports results :rownames no :colnames no :hlines no :cache no
reset
set key autotitle columnheader
set logscale x
set xlabel 'minimum number'
...
#+RESULTS[0e95ab08096d4858fdb0501cb7a047bcec2f3c10]: 1-24-massage2
:results:
...
#+begin_src gnuplot :var data=1-24-massage2 :file fig/1-24-2.png :exports results :rownames no :colnames no :hlines no :cache no :eval no-export
reset
set key autotitle columnheader
set logscale xy
set xlabel 'minimum number'
...
exponentiations. ~expmod~ does the remainder after every exponentiation
