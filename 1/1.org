...
But I wasn't sure where to go from here. I took the opportunity to make [[/1/cc/Ways%20to%20Make%20Changes%20Analyzed.ipynb][some
graphs of the function in Jupyter]] and saw some interesting trends, though I
didn't get any closer to an answer in the process.
...
Let's find out!
#+NAME: 1-15-deps
#+BEGIN_SRC scheme -n :eval no-export :tangle yes :exports code :results value
(define (cube x) (* x x x))
(define (p x) (- (* 3 x) (* 4 (cube x))))
(define (sine angle)
  (if (not (> (abs angle) 0.1))
...
#+RESULTS[883d14195f590de718a5e3a6a9f6a86189e402e8]: 1-15-deps
:results:
| -0.39980345741334 | 5 |
:end:
